...
const EXIT_INVALID_CROP: i32 = 4;
const EXIT_UNSUPPORTED_ASPECT: i32 = 10;
const EXIT_IO_ERROR: i32 = 11;
...
fn extract_xmp_seq_values(xmp: &str, tag_name: &str) -> Option<[f32; 3]> {
    ...
    loop {
        let li_start_rel = match rest.find("<rdf:li>") {
            Some(pos) => pos,
            None => break,
        };
...
fn apply_lenient_xmp_overrides(xmp: &str, metadata: &mut ultrahdr::GainMapMetadata) {
    ...
    if let Some(values) = extract_xmp_seq_values(xmp, "hdrgm:Gamma").or_else(|| {
        extract_xmp_attribute_value(xmp, "hdrgm:Gamma")
            .map(|val| parse_xmp_values_lenient(&val))
    }) {
...
fn default_output_paths(input: &str) -> (String, String) {
...
fn compute_split_rectangles(width: u32, height: u32) -> Result<(Rect, Rect), i32> {
    if height == 0 {
...
fn split_standard_image(args: &Args) -> Result<(), i32> {
...
fn encode_ultrahdr_tile(
    ...
    let (pixel_layout, pixel_data): (PixelLayout, std::borrow::Cow<[u8]>) = match sdr_tile.format {
        PixelFormat::Rgb8 => (PixelLayout::Rgb8Srgb, std::borrow::Cow::Borrowed(&sdr_tile.data)),
        PixelFormat::Rgba8 => {
...
fn try_split_ultrahdr_jpeg(args: &Args) -> Result<UltraHdrSplitOutcome, i32> {
