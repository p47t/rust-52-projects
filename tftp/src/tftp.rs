...
impl LockStep for Receiver {
    fn process(&mut self, packet: &Packet) -> Option<Packet> {
        match packet {
            Packet::WriteRequest { .. } => Some(Packet::Ack { block_num: 0 }),
            Packet::Data { block_num, data } => {
...
mod tests {
    ...
    fn test_packet_parse() {
        let bytes = [
            00u8, 0x01, 0x72, 0x66, 0x63, 0x31, 0x33, 0x35, 0x30, 0x2e, 0x74, 0x78, 0x74, 0x00,
        ...
        let bytes = [
            00u8, 0x02, 0x72, 0x66, 0x63, 0x31, 0x33, 0x35, 0x30, 0x2e, 0x74, 0x78, 0x74, 0x00,
            0x6f, 0x63, 0x74, 0x65, 0x74, 0x00,
        ];
        assert_eq!(
            Packet::from(&bytes),
        ...
        let bytes = [00u8, 0x03, 0x00, 0x01, 0x0a, 0x0a];
        assert_eq!(
            Packet::from(&bytes),
            Some(Packet::Data {
                block_num: 1,
        ...
        let bytes = [00u8, 0x04, 0x00, 0x01];
        assert_eq!(Packet::from(&bytes), Some(Packet::Ack { block_num: 1 }));

        let bytes = [
            00u8, 0x05, 0x00, 0x02, 0x64, 0x65, 0x6e, 0x69, 0x65, 0x64, 0x00,
        ];
        assert_eq!(
            Packet::from(&bytes),
            Some(Packet::Error {
    ...
    fn test_packet_serialization() {
        ...
        let bytes = [00u8, 0x04, 0x00, 0x01];
        assert_eq!(Packet::Ack { block_num: 1 }.to_bytes(), bytes);
    ...
    fn test_receiver() {
        ...
        let reply = receiver.process(&Packet::Data {
            block_num: 2,
            data: vec![],
        });
        assert_eq!(reply, None);
